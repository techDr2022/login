...
interface OnboardingData {
  ...
  basicInfo?: {
    ...
    preferredLanguage?: 'TELUGU' | 'ENGLISH' | 'BOTH'
  }
  // Step 2
  doctors?: any[]
  // Step 3
...
export function ClientOnboardingWizard() {
  ...
  const handleStepSave = async (stepData: any) => {
    ...
    try {
      const normalizedBasicInfo = currentStep === 1
        ? {
            ...stepData,
            doctorOrHospitalName: stepData.doctorOrHospitalName || stepData.name || '',
            location: stepData.location || stepData.addressLine || stepData.city || stepData.area || 'TBD',
          }
        : stepData

      // Create client on first step if not exists
      if (!clientId && currentStep === 1) {
        const client = await createClient({
          name: normalizedBasicInfo.name,
          doctorOrHospitalName: normalizedBasicInfo.doctorOrHospitalName,
          location: normalizedBasicInfo.location,
          services: ['Onboarding'], // Temporary service to satisfy validation
          type: normalizedBasicInfo.type,
        })
        setClientId(client.id)
        await updateClientBasicInfo(client.id, normalizedBasicInfo)
        setData({ ...data, basicInfo: normalizedBasicInfo })
      } else if (clientId) {
        // Update existing client data
        if (currentStep === 1) {
          await updateClientBasicInfo(clientId, normalizedBasicInfo)
          setData({ ...data, basicInfo: normalizedBasicInfo })
        } else {
