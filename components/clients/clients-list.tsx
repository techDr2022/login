...
export function ClientsList() {
  ...
  const handleStatusChange = async (clientId: string, newStatus: ClientStatus) => {
    ...
    try {
      ...
      // Optimistically update the UI so the badge/select updates immediately.
      // Keep the local list consistent with the currently selected `statusFilter`.
      setClients((prev) => {
      ...
      // Ensure the table/grid refresh completes before re-enabling the control
      // (prevents "needs 2-3 clicks" race/out-of-order refresh).
      await fetchClients()
    } catch (err: any) {
