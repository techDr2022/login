...
export const clientMarketingRequirementSchema = z.object({
  ...
  postersPerMonth: z.number().int().min(0).default(0),
  videosPerMonth: z.number().int().min(0).default(0),
  websiteRequirement: z.enum(['NEEDED', 'NOT_NEEDED', 'HAS_OLD_WEBSITE', 'REVAMP']).default('NOT_NEEDED'),
  appointmentBookingRequired: z.boolean().default(false),
  telehealthRequired: z.boolean().default(false),
  gmbStatus: z.enum(['HAVE_ALREADY', 'NEED_TO_CREATE']).default('HAVE_ALREADY'),
  socialCreationStatus: z.enum(['HAVE_ALREADY', 'NEED_TO_CREATE']).default('HAVE_ALREADY'),
  blogsPerMonth: z.number().int().min(0).default(0),
  linkedInCreationRequired: z.boolean().default(false),
