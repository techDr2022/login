...
export async function finalizeClientOnboarding(clientId: string, startDate: Date, accountManagerId?: string) {
  ...
  if (accountManagerId) {
    try {
      ...
      // Auto-create tasks should be "assignedBy" techDr Auto system user (if present).
      // Otherwise fall back to the current user doing onboarding.
      const techDrAutoUser = await (async () => {
        const candidates = await prisma.user.findMany({
          where: { isActive: true, name: { contains: 'techdr', mode: 'insensitive' } },
          select: { id: true, name: true },
        })
        return candidates.find(u => u.name.toLowerCase().includes('auto')) || null
      })()
      const assignedById = techDrAutoUser?.id || session.user.id
