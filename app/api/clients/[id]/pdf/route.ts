...
export async function GET(
  ...
  try {
    ...
    if (client.client_marketing_requirements) {
      ...
      const websiteRequirementLabelMap: Record<string, string> = {
        NEEDED: 'Needed',
        NOT_NEEDED: 'No need',
        HAS_OLD_WEBSITE: 'Old website',
        REVAMP: 'Revamp',
      }
      const presenceLabelMap: Record<string, string> = {
        HAVE_ALREADY: 'Have already',
        NEED_TO_CREATE: 'Need to create',
      }

      addText(`Posters per month: ${marketing.postersPerMonth ?? 0}`, 11)
      addText(`Videos per month: ${marketing.videosPerMonth ?? 0}`, 11)
      addText(`Website: ${websiteRequirementLabelMap[marketing.websiteRequirement] || marketing.websiteRequirement}`, 11)
      addText(`Appointment booking system: ${marketing.appointmentBookingRequired ? 'Needed' : 'No need'}`, 11)
      addText(`Telehealth: ${marketing.telehealthRequired ? 'Needed' : 'No need'}`, 11)
      addText(`GMB: ${presenceLabelMap[marketing.gmbStatus] || marketing.gmbStatus}`, 11)
      addText(
        `Insta/Facebook/YouTube creation: ${presenceLabelMap[marketing.socialCreationStatus] || marketing.socialCreationStatus}`,
        11
      )
      addText(`Blogs per month: ${marketing.blogsPerMonth ?? 0}`, 11)
      addText(`LinkedIn Creation: ${marketing.linkedInCreationRequired ? 'Needed' : 'No need'}`, 11)
      if (marketing.otherServices) {
        addText(`Other Services: ${marketing.otherServices}`, 11)
      }
      if (marketing.notes) {
